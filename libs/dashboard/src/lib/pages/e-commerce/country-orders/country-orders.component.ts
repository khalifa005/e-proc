...
import { CountryOrderData } from '@e-proc/core';
import { NbMediaBreakpoint, NbMediaBreakpointsService, NbThemeService } from '@nebular/theme';
import { takeWhile } from 'rxjs/operators';

@Component({
  selector: 'e-proc-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <e-proc-country-orders-map (selectEvent)="selectCountryById($event)"
                                countryId="USA">
        </e-proc-country-orders-map>
        <e-proc-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  [maxValue]=20>
        </e-proc-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
